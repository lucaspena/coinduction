...
Lemma equivJoinItemR {K} (k : K) {V} (v1 v2 : V) r1 r2 :
  v1 = v2 -> r1 ~= r2 -> (k |-> v1 :* r1) ~= (k |-> v2 :* r2).
intros. subst. auto using equivJoinR.
Qed.

Lemma equivItem {K} (k : K) {V} (v1 v2 : V) :
  v1 = v2 -> k |-> v1 ~= k |-> v2.
...
Ltac find_items :=
  ...
  | [|- MapEquiv ?map (?x |-> _ :* _)] => find x map
      ltac:(fun pf => apply (equivTrans pf);apply equivJoinItemR)
  | [|- MapEquiv ?map (?submap :* _)] => find_submap map submap
      ltac:(fun pf => apply (equivTrans pf);apply equivJoinItemR)
  | [|- ?x |-> ?v1 ~= ?x |-> ?v2] => apply equivItem
  end;[try reflexivity..|try find_items].
