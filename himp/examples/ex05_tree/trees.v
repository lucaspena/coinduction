...
Definition set_kcell kcell c :=
  match c with
    | KCfg _     store stack heap functions alloc_mark =>
      KCfg kcell store stack heap functions alloc_mark
  end.
...
Notation build_node v p := (EBuild ("val" s|-> (v : Exp) :* "next" s|-> (p : Exp))).
...
Fixpoint rep_seg (val : list Z) (tailptr : Z) (p : Z) : MapPattern k k :=
 (match val with
    ...
    | x :: xs => constraint (p <> 0%Z) :*
        (existP p',
           (p h|-> KStruct (Struct ("val" s|-> KInt x :* "next" s|-> KInt p')%Map))
             :* rep_seg xs tailptr p')
  end)%pattern.
Notation rep_list l := (rep_seg l 0%Z).

Local Ltac list_rep_tac := repeat progress (repeat (esplit || eassumption
  || (apply itemP_expose;reflexivity)
  || (instantiate;equate_maps))).
...
list_rep_tac.
eapply IHl1;list_rep_tac.
Qed.

(* A polymorphic list predicate.
   For now, just used as a stack.
   TODO: make seg version, maybe replace plain list prop *)
Fixpoint rep_prop_list {A} (P : A -> Z -> MapPattern k k)
                       (val : list A) (p : Z) : MapPattern k k :=
  (match val with
...
Ltac use_generic_postcondition :=
  ...
  intros k H; destruct k; simpl in H; progress (decompose record H); clear H; subst.
...
Hint Rewrite app_nil_r Z.add_0_r : trans_simpl.
