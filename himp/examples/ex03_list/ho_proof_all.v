...
Proof.
  Ltac T_finish Q := apply T_def with Q;[solve[firstorder using step_mono]|..|assumption];
   ...
   try solve[apply B_def with (Q G);[firstorder using step_mono..|assumption]].
 Ltac T_solve :=
     match goal with
...
(*   eapply sstep;eauto. intros. apply Tf_idem. *)
(*   firstorder. apply T_id. *)
(*   exists Q. split. eapply Tf_id;apply H1;assumption. intros. *)
(*   specialize (H k'0 H3). apply Tf_base. assumption. *)
(* Qed. *)
...
(*       (exists x', cstep x x') -> (forall x', cstep x x' -> T F A x' P) -> T F A x P. *)
(* Proof. *)
(*   intros. apply use_step. assumption. eapply sstep. assumption. assumption. *)
(* Qed. *)
