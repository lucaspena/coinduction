...
Definition kcell_step step k1 k2 :=
  forall env stk heap funs mark,
    step (KCfg k1 env stk heap funs mark)
         (KCfg k2 env stk heap funs mark).
...
Fixpoint cool_exp_exp (hole : Exp) (froze : Exp) {struct froze} :=
 ...
 | EBuild m => EBuild (cool_exp_kmap hole m)
 | EProject e f => EProject (cool_exp_exp hole e) f
 | ECall f es => ECall f
     ((fix cool_exp_exps l :=
...
Definition cool_exp_stmt (hole : Exp) (froze : Stmt) :=
 ...
 | HDealloc e => HDealloc (cool_exp_exp hole e)
...
Definition ext (c : cell) (p : pat) : pat :=
  match p with
    | KPat k env stk heap code funs mark => 
      match c with
        | k_cell k => KPat k env stk heap code funs mark
        | env_cell env =>  KPat k env stk heap code funs mark
        | stk_cell stk =>  KPat k env stk heap code funs mark
        | heap_cell heap =>  KPat k env stk heap code funs mark
        | fun_cell funs =>  KPat k env stk heap code funs mark
        | mark_cell mark =>  KPat k env stk heap code funs mark
      end
  end.
...
Definition mk_step (step : kcfg -> kcfg -> Prop) (p : pat) : Prop :=
  match p with
    | KPat k env stk heap code funs mark =>
      ...
      arg funs (fun f1 f2 =>
      arg mark (fun m1 m2 =>
        step (KCfg k1 e1 s1 h1 f1 m1) (KCfg k2 e2 s2 h2 f2 m2)))))))
  end.
...
Definition krule (step : kcfg -> kcfg -> Prop) (members : list cell) : Prop :=
  mkpat step (KPat wild wild wild wild wild wild wild) members.
