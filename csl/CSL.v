...
Require Import Decidable Peano_dec.

Require Import "../himp/examples/ex03_list/ho_proof_until_gen".
Require Import "../himp/examples/ex03_list/until_all_ok".
...
Definition valueOf (o : loop_outcome nat) :=
  match o with
  | Done v => v
  | Again v => v
  end.
...
| Return (r : nat) : cmd
| Fail : cmd
| Bind (c1 : cmd) (c2 : nat -> cmd) : cmd
| Loop (init : nat) (body : nat -> cmd) : cmd

| Read (a : nat) : cmd
| Write (a v : nat) : cmd
| Lock (a : nat) : cmd
| Unlock (a : nat) : cmd
| Alloc (numWords : nat) : cmd
| Free (base numWords : nat) : cmd
...
| StepBindRecur : forall (c1 c1' : cmd) (c2 : nat -> cmd) h l h' l',
  step (Cfg h l c1) (Cfg h' l' c1')
  -> step (Cfg h l (Bind c1 c2)) (Cfg h' l' (Bind c1' c2))
| StepBindProceed : forall (v : nat) (c2 : nat -> cmd) h l,
  step (Cfg h l (Bind (Return v) c2)) (Cfg h l (c2 v))

| StepLoop : forall (init : nat) (body : nat -> cmd) h l,
  step (Cfg h l (Loop init body)) (Cfg h l (o <- body init; match o with
                                                     | Done a => Return a
                                                     | Again a => Loop a body
                                                     end))
...
| StepRead : forall h l a v,
  h $? a = Some v
  -> step (Cfg h l (Read a)) (Cfg h l (Return v))
| StepWrite : forall h l a v v',
  h $? a = Some v
  -> step (Cfg h l (Write a v')) (Cfg (h $+ (a, v')) l (Return tt))
| StepAlloc : forall h l numWords a,
  a <> 0
  -> (forall i, i < numWords -> h $? (a + i) = None)
  -> step (Cfg h l (Alloc numWords)) (Cfg (initialize h a numWords) l (Return a))
| StepFree : forall h l a numWords,
  step (Cfg h l (Free a numWords)) (Cfg (deallocate h a numWords) l (Return tt))

| StepLock : forall h l a,
  ~a \in l
  -> step (Cfg h l (Lock a)) (Cfg h (l \cup {a}) (Return tt))
| StepUnlock : forall h l a,
  a \in l
  -> step (Cfg h l (Unlock a)) (Cfg h (l \setminus {a}) (Return tt))

| StepPar1 : forall h l c1 c2 h' l' c1',
  step (Cfg h l c1) (Cfg h' l' c1')
  -> step (Cfg h l (Par c1 c2)) (Cfg h' l' (Par c1' c2))
| StepPar2 : forall h l c1 c2 h' l' c2',
  step (Cfg h l c2) (Cfg h' l' c2')
  -> step (Cfg h l (Par c1 c2)) (Cfg h' l' (Par c1 c2')).
...
Definition test := Spec (cfg nat).

Definition init_cfg := Cfg ($0 $+ (0,0)) {} incrementer_par.

Example my_cfg x y := Cfg x y incrementer_par.

Definition test2 x y := (eq (my_cfg x y)).
Print test2.

Definition test3 A (x y : cfg A) := eq x y.
Print test3.
...
Definition myptsto A (a v : nat) : cfg A -> Prop :=
  fun cfg => forall h l c, cfg = (Cfg h l c) /\ cfg = (Cfg ($0 $+ (a, v)) l c).
...
| claim1 : forall h l P,
    B (Cfg h l (P || incrementer)) (fun _ c => inc_inv c) (fun _ => True) ->
    ho_spec B (Cfg h l incrementer) (fun _ c => inc_inv c) (fun _ => True).

Inductive nonho_spec : Spec (cfg unit) :=
| nonho_claim : forall h l,
    nonho_spec (Cfg h l incrementer) (fun _ c => inc_inv c) (fun _ => True).

Print ho_spec.
Print Spec.

Locate sound. Print sound.
Lemma ho_ok : forall (A : Set), sound step nonho_spec.


Inductive ho_spec (A : Set) (B : Spec (cfg A)) : Spec (cfg A) :=
(*  | claim1 : forall x y, ho_spec B (my_cfg x y) test3 (test2 x y).*)
 | claim_inv : 
| claim1 : forall (h : heap) (l : locks) (c : cmd A) (m : fmap locks nat) (x : nat)
                  (P : nat -> option nat -> cfg A -> Prop) (k : nat),
    (P k None (Cfg h l c) /\ forall h' l' c',
        step (Cfg h l c) (Cfg h' l' c') ->
        P k None (Cfg h' l' c') \/
        (~0 \in l /\ m $? l = Some x /\ 0 \in l' /\ P k (Some (S x)) (Cfg h' l' c'))) ->
    ho_spec B (Cfg h l c) step (P k None).
...
Proof.
  ...
  apply HtStrengthen with (fun r => a |-> r * [| r = v|])%sep.
  apply HtRead.
  simp.
  cancel; auto.
  subst; cancel.
 ...
 * needed to maintain the property in this case, but it's a pleasant starti
